...
public class MyBatisGeneratorConfigurationParser {
    ...
    private void parseContext(Configuration configuration, Node node) {
        ...
        Properties attributes = parseAttributes(node);
        // 这个属性用来设置生成对象类型的默认值, 对象类型定义了MBG如何生成实体类,对某些对象类型，MBG会给每一个表生成一个单独的实体类。
        ...
        //  然后为所有其他的字段生成一个单独的实体类。 MBG会在所有生成的实体类之间维护一个继承关系（注：BLOB类 继承 其他字段类 继承 主键类）。
    ...
    protected void parseTable(Context context, Node node) {
        ...
        // 数据库表的名称(不包括schema或catalog)。如果需要，指定的值可以包含SQL通配符。
        ...
        // 如果指定，这个值会用在生成的select查询SQL的表的别名和列名上
